...
function postStartAction()
{
    ...
    if [[ "$DEV" && $DATABASE_TYPE != "dpudb" ]]; then
        ...
        SYSCTL_NET_CONFIG="/usr/lib/sysctl.d/90-sonic.conf"
	...
	waitForAllInstanceDatabaseConfigJsonFilesReady
        ...
        # DPU database instances must not touch the host CONFIG_DB_INITIALIZED flag.
        # Only the main NPU database instance should manage this key to avoid a race
        # where DPU instances overwrite CONFIG_DB_INITIALIZED back to 0 after
        # config-setup has already set it to 1, causing swss startup to fail.
        if [[ "$DATABASE_TYPE" != "dpudb" ]]; then
            if [ -e /etc/sonic/pending_config_migration ] || [ -e /etc/sonic/pending_config_initialization ]; then
                ...
                # For first boot/fast/cold reboot case, DB contains nothing at this point
                ...
                $SONIC_DB_CLI CONFIG_DB SET "CONFIG_DB_INITIALIZED" "0"
                # this is not a first time boot to a new image. Database container starts w/ old pre-existing config
...
start() {
    ...
    if [[ -z "$DEV" || $DATABASE_TYPE == "dpudb" || "$DEV" == *"dpu"* ]]; then
        NET="host"
