...
start() {
    ...
    docker create -t -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro -v /var/run/dbus:/var/run/dbus:rw -v /:/mnt/host:ro -v /tmp:/mnt/host/tmp:rw -v /var/tmp:/mnt/host/var/tmp:rw --pid=host --cap-add=SYS_ADMIN --cap-add=SYS_BOOT --cap-add=SYS_PTRACE --cap-add=NET_ADMIN --cap-add=DAC_OVERRIDE --security-opt apparmor=unconfined --security-opt seccomp=unconfined --userns=host -v /var/run/gnmi:/var/run/gnmi:rw \
