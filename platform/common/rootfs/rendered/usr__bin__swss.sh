...
start() {
    ...
    CREATE_ONLY_CONFIG_DB_BUFFERS_JSON_FOUND=false

    PLATFORM_FOLDER="/usr/share/sonic/device/$PLATFORM"
    if [ -d "$PLATFORM_FOLDER" ]; then
        if [ -f "$PLATFORM_FOLDER/create_only_config_db_buffers.json" ]; then
    ...
    docker create -t --cap-add=NET_ADMIN --security-opt apparmor=unconfined --security-opt=systempaths=unconfined -v /etc/network/interfaces:/etc/network/interfaces:ro -v /etc/localtime:/etc/localtime:ro  -v /etc/network/interfaces.d/:/etc/network/interfaces.d/:ro -v /host/machine.conf:/host/machine.conf:ro -v /etc/sonic:/etc/sonic:ro -v /var/log/swss:/var/log/swss:rw -v /zmq_swss:/zmq_swss:rw \
