...
start() {
    ...
    docker create -t --pid=host --cap-add=SYS_ADMIN --cap-add=SYS_PTRACE --cap-add=DAC_OVERRIDE --security-opt apparmor=unconfined --security-opt seccomp=unconfined -v /lib/systemd/system:/lib/systemd/system:rw -v /etc/audit:/etc/audit:rw -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro \
