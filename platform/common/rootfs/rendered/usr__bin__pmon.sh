...
start() {
    ...
    docker create --cap-add=SYS_RAWIO --cap-add=SYS_ADMIN -t --security-opt apparmor=unconfined --security-opt=systempaths=unconfined -v /etc/sonic:/etc/sonic:ro -v /etc/localtime:/etc/localtime:ro  -v /host/reboot-cause:/host/reboot-cause:rw -v /host/pmon/stormond:/usr/share/stormond:rw -v /var/run/platform_cache:/var/run/platform_cache:ro -v /usr/share/sonic/device/pddf:/usr/share/sonic/device/pddf:ro -v /var/lock/pddf-locks:/var/lock/pddf-locks:rw -v /sys/:/sys/:rw \
